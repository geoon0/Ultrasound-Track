...
static const int kLedPin    = 7; // 데이터 수집 상태 표시용 LED 핀 (원하시는 핀으로 변경 가능)
...
void setup() {
  ...
  // [임시 진단] LED 하드웨어 테스트 — 부팅 시 5번 깜빡인다. 확인 후 이 for 블록만 삭제하면 됨.
  for (int i = 0; i < 5; i++) {
    digitalWrite(kLedPin, HIGH); delay(200);
    digitalWrite(kLedPin, LOW);  delay(200);
  }
}
...
void loop() {
  // 파이썬 수집 스크립트에서 보내는 신호 읽기 (S: 시작, E: 종료)
  while (Serial.available() > 0) {
    char c = Serial.read();
    if (c == 'S') digitalWrite(kLedPin, HIGH);
    else if (c == 'E') digitalWrite(kLedPin, LOW);
  }
